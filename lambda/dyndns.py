            ...
            'Changes': [
                {
                    'Action': 'UPSERT',
                    'ResourceRecordSet': {
                        'Name': "%s.%s." % (record['instanceId'], os.environ['ZONE_DOMAIN']),
                        'Type': 'A',
                        'TTL': 300,
                        'ResourceRecords': [
                            {
                ...
                },
                {
                    'Action': 'UPSERT',
                    'ResourceRecordSet': {
                        'Name': "%s.%s." % (record['hostName'], os.environ['ZONE_DOMAIN']),
                        'Type': 'A',
                        'TTL': 300,
                        'ResourceRecords': [
                            {
...
def lambda_handler(event, context):
    ...
    if instanceState == 'running':
        ...
        writeLog('Unhandled instance state "%s": i-%s' % (instanceState, instanceId))
